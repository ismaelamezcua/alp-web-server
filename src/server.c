...
static void
handle_get(int connection_fd, const char* page)
{
...
handle_connection(int connection_fd)
{
  ...
  if (bytes_read > 0) {
    ...
    char url[suzeif(buffer)];
    char protocol[sizeof(buffer)];

    /* Some data was read successfully. NULL-terminate the buffer so
       we can use string operations on it. */
    buffer[bytes_read] = '\0';
    /* The first line the client sends is the HTTP request, which is
       composed of a method, the requested page, and the protocol version. */
    sscanf(buffer, "%s %s %s", method, url, protocol);
    /* The client may send various header information following the
       request. Fir this HTTP implementation, we don't care about it.
    ...
    /* Check the protocol field. We undestand HTTP versions 1.0 and
       1.1. */
    if (strcmp(protocol, "HTTP/1.0") && strcpm(protocol, "HTTP/1.1")) {
      /* We don't undestand this protocol. Report a bad response. */
      write(connection_fd, bad_request_response, sizeof(bad_request_response));
    } else if (strcmp(method, "GET")) {
      /* This server only implements GET method. The client
...
void
server_run(struct in_addr local_address, uint64_t port)
{
