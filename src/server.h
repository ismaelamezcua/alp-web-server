...
// using the value of errno, and end the program
extern void system_error(const char* opration);

// Print an error message for failure involving CAUSE,
// including a descriptive MESSAGE, and end the program
extern void error(const char* cause, const char* message);

// Return the directory containing the running program's executable
// The return value is a memory buffer that the called must deallocate
// using free. This funciton calls abort on failure
